...
<html style="height: 8365px; width: 719px; ">
<head>
...
<div class="w loc">
	<div class="h-title">
		<div class="h-logo"><a href="http://localhost:8080/web/front_page/product/product.jsp"><img src="/res/img/pic/logo-1.png" /></a></div>
	    <div class="h-search">
	      	<input type="text" />
				...
				<c:if test="${!empty brands}">
					<li><b>品牌：</b><p>
						...
						<c:forEach items="${brands}" var="brand">
							<a href="javascript:void(0);" onclick="window.location.href='/product/display/list.shtml?brandId=${brand.id}&brandName=${brand.name}&typeId=${typeId}&typeName=${typeName}'
				...
				<c:if test="${!empty types}">
					<li><b>类型：</b><p>
						...
						<c:forEach items="${types}" var="type">
							<a href="javascript:void(0);" onclick="window.location.href='/product/display/list.shtml?typeId=${type.id}&typeName=${type.name}&brandId=${brandId}&brandName=${brandName}'" title="${type.name}">${type.name}</a>
						</c:forEach>
					</p></li>
				</c:if>
